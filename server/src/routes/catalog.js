...
router.post('/', async (req, res) => {
  try {
    ...
    const stmt = await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    const result = await stmt.run(sku, name, description, category, unit || 'each', asCents(unit_price), preferred_supplier_id, lead_time_days || 3, image_url || '📦', resolvedType);
    const created = await db.prepare(`SELECT * FROM catalog_items WHERE id = ?`).get(result.lastInsertRowid);
